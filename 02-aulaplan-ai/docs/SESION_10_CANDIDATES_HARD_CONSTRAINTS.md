[Repositorio](../../README.md) · [Proyecto](../README.md) · [Índice](./README.md) · [← Sesión 09](./SESION_09_SCHEDULER_DOMAIN_LOADER.md) · [Sesión 11 →](./SESION_11_BACKTRACKING_SOLVER.md)

# Sesión 10 — Candidatos + Hard Constraints

**Duración:** 1 h 30 min  
**Objetivo:** generar únicamente combinaciones potencialmente válidas.

## 1. Expandir ofertas

```text
Offering o1 con weekly_blocks=2
  ↓
o1:1
o1:2
```
...
### `apps/api/src/scheduler/candidates.py`
...
hard group unavailable
room.capacity insuficiente
room.type incorrecto
```
...
for unit in expand_units(context):
    print(unit.id, len(build_candidates(unit, context)))
```

Una unidad con `0` candidatos hace imposible el horario.
...
- [ ] hard constraints filtradas.

## Commit

```bash
git add .
git commit -m "feat: build scheduler candidates and hard filters"
```
